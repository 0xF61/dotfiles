...
webcam_shot(int async) {
  ...
  int r = snprintf(
    cmd,
    CMD_LENGTH,
    "ffmpeg -y -loglevel quiet -f video4linux2 -i /dev/video0 -frames:v 1 -f image2 %s/slock%d.jpg%s",
    getenv("HOME"),
    lock_tries,
    async ? " &" : ""
  );
...
// Send an SMS via Telegram.
static int
telegram_send(const char *msg) {
#if TELEGRAM_SEND
  const char *HOME = getenv("HOME");
  char cmd[CMD_LENGTH];

  // Send the SMS/MMS via TELEGRAM
  int r = snprintf(
    cmd,
    CMD_LENGTH,
    "curl 'https://api.telegram.org/bot000000000:00000000000000000000000000000000000/sendPhoto?chat_id=000000000' -F 'photo=@%s/slock%d.jpg'",
    HOME,
    lock_tries
  );
...
play_beep(int async) {
  ...
  int r = snprintf(
    cmd,
    CMD_LENGTH,
    "mpv %s/.config/beep.wav 2> /dev/null%s",
    getenv("HOME"),
    async ? " &" : ""
  );
  ...
  while (running && !XNextEvent(dpy, &ev)) {
    ...
    switch(ksym) {
      case XK_Return: {
        ...
        if (running) {
          ...
          if (lock_tries > 5) {
            ...
            // Send an SMS/MMS via telegram.
            telegram_send("Bad screenlock password.");
      ...
      case XK_F13: {
        ...
        // Send an SMS/MMS via telegram.
        telegram_send("Bad screenlock key.");
